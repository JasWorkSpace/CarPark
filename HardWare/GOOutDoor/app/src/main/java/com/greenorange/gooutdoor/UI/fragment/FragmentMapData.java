...
public class FragmentMapData extends BaseFragment {
    ...
    public void onViewCreated(View view, Bundle savedInstanceState) {
        ...
        mMapDataInfo = (MapDataInfo) view.findViewById(R.id.mapdata);
    ...
    private class DragableGridviewAdapter extends BaseAdapter implements DragableGridview.OnItemClickListener, DragableGridview.OnSwappingListener
        ...
        private final static float MINUTE = 60.0f;
        private final static float HOUR   = 3600.0f;
            ...
            if (userSportsData != null) {
                ...
                for (Item item : mMapDataList) {
                    ...
                    if (item.mId == ID_MAPDATA_CALORIE) {
                        ...
                        item.mCard.setValue(FormatUtils.getFloat3to2(context, totleDistance / 1000.0f));
                    } else if (item.mId == ID_MAPDATA_SPEED) {
                        if (Card.STATE_V1 == item.mCard.getSTATE()) {
                            float v1 = (totleTime > 0 ? (HOUR * totleDistance / totleTime) : 0.0f);
                            item.mCard.setValue(FormatUtils.getFloat3to2(context, v1));
                        } else {
                            float v2 = (totleDistance > 0 ? (totleTime / totleDistance / MINUTE) : 0.0f);
                            item.mCard.setValue(FormatUtils.getFloat3to2(context, v2));
                        }
