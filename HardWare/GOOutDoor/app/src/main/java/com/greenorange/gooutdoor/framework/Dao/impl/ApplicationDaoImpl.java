...
public class ApplicationDaoImpl extends BroadcastReceiver implements ApplicationDao{
    ...
    public boolean updateUser() {
        if(mUserData != null){
            UserDBData userDBData = UserDBHelper.getUserDataByUserId(GOApplication.getInstance(), mUserData.getUserid());
            if(userDBData != null){
                mUserData = userDBData;
                return true;
            }
        }
        return false;
...
//        intentFilter.addAction(Intent.ACTION_SCREEN_ON);
        ...
        //init front
        if(Feature.isFointSupport()) {
            CalligraphyConfig.initDefault(new CalligraphyConfig.Builder()
                            .setDefaultFontPath(context.getString(R.string.config_default_fontpath))
