...
public abstract class BaseMapFragment extends BaseFragment implements iMapScreenShotListener, View.OnClickListener {
            ...
            case EventID.ID_STATE_FloatControlMenu:{
                if(eventStateChange.viewID == FloatControlMenu.ID_STATE_MENU) {
                    if (FloatControlMenu.MENU_STATE_CLOSE == eventStateChange.newState) {
                        setCurrentLocationIconVisible(true);
                    } else if (FloatControlMenu.MENU_STATE_OPEN == eventStateChange.newState) {
                        setCurrentLocationIconVisible(false);
                    }
                }
            }break;
        }
    ...
    private synchronized void setCurrentLocationIconVisible(final boolean show){
        if(icon_showCurrentLocation != null
                && show != (icon_showCurrentLocation.getVisibility() == View.VISIBLE)){
            new Runnable(){
                @Override
                public void run() {
                    AnimatorUtils.startScaleAnimation(show, 200, icon_showCurrentLocation);
                }
            }.run();
        }
    }
