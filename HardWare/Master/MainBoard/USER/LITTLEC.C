...
enum tokens {ARG, CHAR, INT, IF, ELSE, FOR, DO, WHILE,
             SWITCH, RETURN, EOL, FINISHED, VAR,END};
...
/*
	Parser file defination
*/


/*
	自定义库函数，函数声明一定要在intern_func_type定义之前
			*/
int call_getch(void), call_putch(void); 
...
//		exit(1); 
	if(setjmp(e_buf)) return(1); /* initialize long jump buffer */
