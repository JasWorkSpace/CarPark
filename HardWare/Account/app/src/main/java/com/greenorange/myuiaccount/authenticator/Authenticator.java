...
class Authenticator extends AbstractAccountAuthenticator {
    ...
    public Bundle addAccount(AccountAuthenticatorResponse response, String accountType,
        ...
        Log.d("addAccount()");
        final Intent intent = Utils.createLogainIntent(mContext);
    ...
    public Bundle getAuthToken(AccountAuthenticatorResponse response, Account account,
        ...
        // return an error
        if (!authTokenType.equals(AuthenticatorConfig.AUTHTOKEN_TYPE)) {
        ...
        if (password != null) {
            try {//load service.
                UserInfo userInfo = NetworkUtilities.getUserInfo(mContext, account.name, ServiceHelper.getDecrypt(password));
                if (userInfo != null) {
                    String authToken = (new Gson().toJson(userInfo));//ServiceHelper(account.name, password);
                    if (!TextUtils.isEmpty(authToken)) {
                        final Bundle result = new Bundle();
                        result.putString(AccountManager.KEY_ACCOUNT_NAME, account.name);
                        result.putString(AccountManager.KEY_ACCOUNT_TYPE, AuthenticatorConfig.ACCOUNT_TYPE);
                        try {
                            result.putString(AccountManager.KEY_AUTHTOKEN,//encypt and encoding
                                    ServiceHelper.getEncodeParam(ServiceHelper.getClientEncrypt(authToken)));
                        }catch (Throwable e){e.printStackTrace();
                            Log.d("getAuthToken KEY_AUTHTOKEN fail-->"+e.toString());
                        }
                        return result;
                    }
                }
            }catch (Throwable e){e.printStackTrace();
                Log.d("getAuthToken  fail-->"+e.toString());
            }
        }
